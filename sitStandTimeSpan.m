...
clear
Fs = 1e1;
[allAddr, allTs] = loadaerdat('2_255_B (1).aedat');
intensityOverTime = eventIntensityOverTime(allTs, Fs);

threshold=250; % Difference threshold, once the difference between two consecutive insensities is above/below this number, consider it the start/end index
...
[Minima, MinIdx] = findpeaks(intensityOverTime_inverse);
% True minima
% Minima = intensityOverTime(MinIdx);

Minima = 1.01 * max(Maxima) - Minima;

threshold = 750;

index = find(Minima < threshold);
...
clear
Fs = 8 * 1e1;
[allAddr, allTs] = loadaerdat('1_255_B.aedat');
% Moving Average Filter
intensityOverTime = eventIntensityOverTime(allTs, Fs);

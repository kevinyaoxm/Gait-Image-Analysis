...
inputRGBVideo = VideoReader('test_data2/SitStand.mp4');
...
while hasFrame(inputRGBVideo)
    ...
    img = readFrame(inputRGBVideo);
      ...
      fullname = fullfile('test_data2','images',filename);
      ...
      yuv_fullname = fullfile('test_data2','yuv_images',yuv_filename);
...
for i=1:count
   ...
   img_prev = double(imread(fullfile('test_data2','yuv_images',imageIndexName_prev)));
   img_next = double(imread(fullfile('test_data2','yuv_images',imageIndexName_next)));
   ...
   diff_fullname2 = fullfile('test_data2','diff_threshold',diff_filename2);
...
for i=1:count
   ...
   % Read images from files
   imageIndexName = [sprintf('%03d',i) '.jpg'];
   img = double(imread(fullfile('test_data2','diff_threshold',imageIndexName)));
   ...
   diff_filename = [sprintf('%03d',i) '.jpg'];
   diff_fullname = fullfile('test_data2','diff_mf_threshold2',diff_filename);
   imwrite(uint8(img_mf),diff_fullname);
end
...
for iter=1:count
    ...
    img_stack = double(imread(fullfile('test_data2','diff_mf_threshold2',imageIndexName1)));
        
    for iter2=2:15
        i = i+1;
        % Read images from files
        imageIndexName = [sprintf('%03d',i) '.jpg'];
        img = double(imread(fullfile('test_data2','diff_mf_threshold2',imageIndexName)));
    ...
    for iter2=1:15
        i = i+1;
        diff_filename = [sprintf('%03d',i) '.jpg'];
        diff_fullname = fullfile('test_data2','diff_mf_threshold3',diff_filename);
...
for itera=1:count
    ...
    for i=1:length(horizontal_sum)-2; % Loop through vector intensityOverTime - 1 to avoid accessing past the vector
        difference=abs(horizontal_sum(i+1)-horizontal_sum(i)); % calculate difference between neighboring intensities
        if difference > peak_threshold && (horizontal_sum(i+2) > 5000) % If statement to determine start time index
    ...
    for i=length(horizontal_sum):-1:3; % Loop through vector intensityOverTime - 1 to avoid accessing past the vector
        ...
        if difference > peak_threshold_rev  && (horizontal_sum(i-2) > 5000)  % If statement to determine start time index
